...
#define MAX_PARENTS_FOR_AGENT 4   /* k = max candidate parents */

/* Raw counters/state */
uint32_t status_gen_count      = 0;      /* packets generated by this node        */
uint32_t status_fwd_count      = 0;      /* packets enqueued (own+child)          */
uint32_t status_qloss_count    = 0;      /* MAC drops due to queue full           */
double   status_residual_energy= INIT_ENERGY_J;
uint16_t status_rank           = 0;      /* RPL rank (hop proxy)                  */

/* Derived metrics */
double   status_bdi            = 0.0;    /* 1 - residual/INIT_ENERGY_J            */
double   status_qlr            = 0.0;    /* qloss / fwd                           */
double   status_wr             = 0.0;    /* fwd / gen                             */
uint16_t status_pc             = 0;      /* parent count                          */
uint32_t status_parent_switches= 0;      /* stability: #times parent changed      */

/* Neighbor candidate table (for agent request) */
...
/* Agent handshake */
uint8_t  agent_waiting         = 0;      /* 1 = block until controller responds   */
uint16_t agent_parent          = 0;      /* chosen parent ID (controller writes)  */
...
/* Refresh all status_* globals */
static void
refresh_status(void) {
  /* Raw counters (replace state.* with your struct if named differently) */
...
/* ------------------------------------------------------------------ */
/* Ask agent for parent (blocking until controller responds)          */
/* ------------------------------------------------------------------ */
