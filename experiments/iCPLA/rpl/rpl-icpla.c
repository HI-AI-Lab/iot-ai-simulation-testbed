...
#define N_A   6u /* actions 0..5 tenths (reserved for future use) */
...
{
  (void)etx; (void)ecr; (void)e2e_ms; (void)now_ms;

  /* We keep the packed state available for future extensions */
  uint16_t s = pack_state(qlr, etx, 0.0f);
  (void)s;

  /* Simple policy: higher QLR -> increase dropping; lower QLR -> decrease */
  if(qlr > 0.4f) {
    act = (act < 5) ? (act + 1) : 5;
  } else if(qlr < 0.1f) {
    act = (act > 0) ? (act - 1) : 0;
