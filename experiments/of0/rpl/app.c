...
/* ------------------ Packet header & sink-only accumulators ------------------ */
...
/* -----------------------------------------------------------------------------
 * Receiver callback (all nodes receive, root logs global stats)
 * -------------------------------------------------------------------------- */
static void
        ...
        uint16_t datalen)
{
  ...
  if(node_id == 1 && data != NULL && datalen >= (int)sizeof(pkt_hdr_t)) {
    ...
    if(hdr.src_id < MAX_NODES_TRACKED) {
      ...
      float prr_global = (expected_total > 0) ?
                         ((float)sink_recv_total / (float)expected_total) : 0.0f;
      float e2e_avg_ms = (sink_e2e_samples > 0) ?
                         ((float)sink_e2e_sum_ms / (float)sink_e2e_samples) : 0.0f;

      LOG_INFO("METRIC PRR_GLOBAL prr=%.3f recv=%lu expected=%lu\n",
               prr_global,
               (unsigned long)sink_recv_total,
               (unsigned long)expected_total);
      LOG_INFO("METRIC E2E avg_ms=%.2f samples=%lu\n",
               e2e_avg_ms,
               (unsigned long)sink_e2e_samples);
    }
  }
}
 ...
 * Main process
 * -------------------------------------------------------------------------- */
PROCESS(app_process, "UDP app with metrics (OF0)");
AUTOSTART_PROCESSES(&app_process);

PROCESS_THREAD(app_process, ev, data)
{
  static uint8_t payload[PAYLOAD_SIZE];
  static uip_ipaddr_t dest_ip;
  ...
  while(1) {
    ...
    if(NETSTACK_ROUTING.node_is_reachable() &&
       NETSTACK_ROUTING.get_root_ipaddr(&dest_ip)) {
      ...
      if(r < DROP_PROB_PER_TEN) {
        ...
        LOG_INFO("DEBUG SEND: node=%u seq=%lu\n", node_id, (unsigned long)hdr.seq);

        simple_udp_sendto(&udp_conn, payload, sizeof(hdr), &dest_ip);
        generated++;
      }
    }
